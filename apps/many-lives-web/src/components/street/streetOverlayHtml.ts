...
import {
  buildRowanFallbackNotebookModel,
  firstAfternoonFieldNoteTitle,
  firstAfternoonMaraAdaLeadFieldNoteNextCopy,
  type FirstAfternoonFieldNoteKey,
...
export function buildMindTabHtml(options: {
  ...
  const notebook = buildRowanFallbackNotebookModel({
    game,
    primaryPerson,
    primaryPlace,
    primaryThread,
...
export function buildFirstAfternoonFieldNoteHtml(
  ...
  type FirstAfternoonFieldNoteEntry = {
    key: FirstAfternoonFieldNoteKey;
    note: NonNullable<StreetGameState["firstAfternoon"]>["leadFieldNote"];
    title: string;
  };
  const candidateNotes: FirstAfternoonFieldNoteEntry[] = [
    {
      key: "mara-ada-lead",
      note: game.firstAfternoon?.leadFieldNote,
      title: firstAfternoonFieldNoteTitle("mara-ada-lead"),
    },
    {
      key: "first-afternoon",
      note: game.firstAfternoon?.fieldNote,
      title: firstAfternoonFieldNoteTitle("first-afternoon"),
    },
  ];
  const notes = candidateNotes.filter((entry) => entry.note);
...
function fieldNoteNextCopy(
  ...
  return firstAfternoonMaraAdaLeadFieldNoteNextCopy(game, fallback);
}
